...
public class ScanExample {
	...
	public static void main(String[] args) throws IOException {
		...
		Scan scan1 = new Scan(); // co ScanExample-1-NewScan Create empty Scan
									// instance.
		ResultScanner scanner1 = table.getScanner(scan1); // co
		...
		scanner1.close(); // co ScanExample-4-Close Close scanner to free remote
							// resources.
		...
		// vv ScanExample
		Scan scan2 = new Scan();
		scan2.addFamily(Bytes.toBytes("colfam1")); // co ScanExample-5-AddColFam
													// Add one column family
													// only, this will suppress
													// the retrieval of
													// "colfam2".
		...
		scan3.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("col-5"))
				...
				. // co ScanExample-6-Build Use builder pattern to add very
					// specific details to the Scan.
		...
		ResultScanner scanner4 = table.getScanner(Bytes.toBytes("colfam1"));
		Result result = null;
		while((result = scanner4.next()) != null ){
